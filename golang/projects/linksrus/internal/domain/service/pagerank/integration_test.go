...
import (
	...
	pr "github.com/enesanbar/workspace/projects/linkrus/internal/domain/pagerank"
...
func (s *DistributedPageRankTestSuite) runStandaloneCalculator(c *gc.C, graphInstance graph.Graph) map[uuid.UUID]float64 {
	calc, err := pr.NewCalculator(pr.Config{ComputeWorkers: 1})
	c.Assert(err, gc.IsNil)
...
func (s *DistributedPageRankTestSuite) runDistributedCalculator(c *gc.C, graphInstance graph.Graph, indexInstance index.Indexer, numWorkers int) map[uuid.UUID]float64 {
	...
	master, err := pagerank.NewMasterNode(pagerank.MasterConfig{
		ListenAddress:  ":9998",
		Clock:          clk,
	...
	for i := 0; i < numWorkers; i++ {
		go func(i int) {
			defer wg.Done()
			worker, err := pagerank.NewWorkerNode(pagerank.WorkerConfig{
				MasterEndpoint:    ":9998",
