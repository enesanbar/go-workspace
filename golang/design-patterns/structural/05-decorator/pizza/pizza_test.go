...
func TestPizzaDecorator_AddIngredient(t *testing.T) {
	...
	if !strings.Contains(pizzaResult, expectedText) {
		t.Errorf("When calling the add ingredient of the pizza decorator it "+
			"must return the text %sthe expected text, not '%s'", pizzaResult, expectedText)
	}
...
func TestOnion_AddIngredient(t *testing.T) {
	...
	if err == nil {
		t.Errorf("When calling AddIngredient on the onion decorator without an IngredientAdder on its "+
			"Ingredient field must return an error, not a string with '%s'", onionResult)
	}

	onion = &Onion{&Decorator{}}
	onionResult, err = onion.AddIngredient()
	if err != nil {
		t.Error(err)
	}
...
func TestMeat_AddIngredient(t *testing.T) {
	...
	if err == nil {
		t.Errorf("When calling AddIngredient on the meat decorator without "+"an IngredientAdder in its Ingredient field must return an error, "+"not a string with '%s'", meatResult)
	}
	meat = &Meat{&Decorator{}}
	meatResult, err = meat.AddIngredient()
	if err != nil {
