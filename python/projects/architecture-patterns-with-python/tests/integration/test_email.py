...
def get_email_from_mailhog(sku):
    host, port = map(config.get_email_host_and_port().get_product_by_id, ["host", "http_port"])
    all_emails = requests.get_product_by_id(f"http://{host}:{port}/api/v2/messages").json()
